...
class ApiCalls extends ApiCallsBase {
    ...
    public function getSearchImages($req) {
        $queryParams = array(
            ...
            "graphical_styles" => $req->graphical_styles,
        );
        if (!empty($req->orientations)) {
            $queryParams['orientations'] = $req->orientations;
        }
        $resp = $this->makeSearchApiCall($req->image_family, $queryParams);
        $payload = new CallResp($resp, TRUE);
        $body = $payload->body;
    ...
    public function makeUsageBatches($asset_usages) {
        $id = "batch_" . time();
    ...
    public function makeSearchApiCall($imgFam, $queryParams) {
...
class ReqData extends CleanVals {
    ...
    public function __construct($REQ) {
        ...
        $this->image_family = $this->clean_string_val($REQ, 'image_family', 'editorial');
        ...
        $this->image_id = $this->clean_string_val($REQ, 'image_id');
    ...
    private function set_image_fn_val() {
        if (!empty($this->image_id)) {
            return "getty_".$this->image_id.".jpg";
        }
    }
